...
- **챗봇** (`api/chat/route.ts`): 질문 임베딩 → `RAG_VECTORIZE` topK 5(>0.3) → 청크 본문은 `public/rag-chunks.json`에서 매핑 → Claude **스트리밍** 호출(ReadableStream). system 프롬프트에 `codebase-summary.txt`와 작성자 소개(`src/data/about.ts`: 경력·성과·사이드프로젝트·스킬)를 prompt caching(ephemeral)으로 상시 포함(about은 RAG가 아니라 항상 주입해 작성자 질문에 안정적으로 답). 검색된 청크의 글을 중복 제거해 `X-Chat-Sources` 헤더로 전달 → 답변 하단 "참고한 글" 링크 칩. 클라이언트(`useChat`)는 응답을 완성된 단어 단위로 표시하고 `ChatMessages`에서 서로 겹치는 fade-in을 적용하며, 마지막 단어가 나타난 뒤 참고 글을 이어서 표시한다(글자 타이핑 없음). 빈 화면엔 추천 질문 칩, 대화는 sessionStorage에 저장돼 새로고침엔 유지되지만 탭을 닫으면 초기화(서버 무저장).
...
- **방문 리포트 추적**: `public/cloudflare-analytics.js`를 root layout의 `next/script`로 로드한다. www·루트에서만 기존 Pages RUM 비콘을 실행해 주간 리포트의 siteTag `f9fe631f1ab8491b94ebc157812b5072`를 유지한다. Preview·localhost는 제외한다. 별도 siteTag의 도메인 자동 삽입은 중지하고 기존 데이터는 보존했다. siteTag와 비콘 token은 서로 다른 식별자다. 실행·외부 I/O는 `weekly-analytics-report.mjs`, Markdown·snapshot 생성은 `weekly-analytics-render.mjs`, 계산식은 `weekly-analytics-metrics.mjs`가 담당한다. 리포트는 Cloudflare 일별 수집 상태와 국가별 전주 증감 기여를 함께 보여주며, 표본 20 미만의 비율 비교를 제한하고 분자 초과·수집 공백·서로 다른 지표 흐름·sampling 변화 폭을 자동 진단한다.
...
- **API 직접 처리**: `workerApi.ts`가 D1 중심 다섯 API와 미디어·검색 API의 캐시 미스·개인별 읽기·mutation을 Next.js 초기화 없이 실행한다. Preview 쓰기 차단이 먼저 적용되고 HEAD/OPTIONS/405는 기존 Next의 HTTP 규칙을 따른다. `X-API-Runtime: worker`로 구분한다. 실패한 mutation은 Next로 재시도하지 않고 500을 반환한다. 미디어 인증·Range·CRUD와 검색의 키워드/벡터 병합 로직은 공유 구현을 사용하며, R2의 바이너리 업로드·이름 변경 복사는 Blob/ReadableStream을 그대로 전달해 전체 파일을 ArrayBuffer로 중복 적재하지 않는다.
