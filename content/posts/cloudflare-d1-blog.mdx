...
SQLite의 ALTER TABLE은 제약이 꽤 있어서, 저도 이걸로 시간을 좀 날렸어요.
...
처음에 `crypto`를 import 했다가 빌드 에러가 나서 당황했는데, Web Crypto API가 거의 동일한 기능을 제공하더라고요. Edge Runtime에서는 Node.js 모듈부터 찾지 말고 Web API를 먼저 확인하는 게 속 편해요.
