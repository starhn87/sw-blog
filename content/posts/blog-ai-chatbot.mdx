...
개인 블로그에 AI 챗봇을 붙여보면서 느낀 건, 생각보다 거창한 인프라가 필요 없다는 거였어요. 처음에는 키워드 매칭만으로도 RAG가 동작했고, 이후 검색 퀄리티를 높이기 위해 Workers AI + Vectorize 기반 시맨틱 검색으로 자연스럽게 전환할 수 있었어요. 스트리밍 대신 JSON + 애니메이션 조합이 오히려 더 자연스러웠고요.
