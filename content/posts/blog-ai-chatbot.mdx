...
단순하지만 챗봇의 RAG 용도로는 충분해요. 참고로 블로그 검색 기능에는 이후 Cloudflare Workers AI(bge-m3) + Vectorize를 도입해서, "admin"과 "어드민"처럼 언어가 달라도 의미 기반으로 매칭되도록 개선했어요. 챗봇 RAG까지 벡터 DB로 전환할 필요가 있을지는 글이 더 많아지면 다시 판단해볼 생각이에요.
...
개인 블로그에 AI 챗봇을 붙여보면서 느낀 건, 생각보다 거창한 인프라가 필요 없다는 거였어요. 챗봇 RAG는 키워드 매칭만으로도 충분히 돌아가고, 스트리밍 대신 JSON + 애니메이션 조합이 오히려 더 자연스러웠어요.
