...
보통은 벡터 DB에 임베딩을 저장하고 유사도 검색을 하는데, 개인 블로그에서 그 정도 인프라는 과하다고 판단했어요. 글이 수백 개도 안 되니까요.
...
단순하지만 개인 블로그 규모에서는 충분해요. 벡터 DB를 쓰면 의미 기반 검색이 가능하겠지만, 외부 의존성이 추가되고 비용도 발생하니까 이 정도가 적절하다고 봤어요.
...
모델은 **Haiku**를 선택했어요. 개인 블로그 챗봇에서 Sonnet이나 Opus 급의 추론 능력은 필요 없고, 빠른 응답 속도와 낮은 비용이 더 중요하다고 판단했어요.
...
인프라를 의심하기 전에 **에러 내용에 충실**하자는 교훈을 얻었어요.
...
개인 블로그에 AI 챗봇을 붙여보면서 느낀 점을 정리하면요.

**벡터 DB 없이도 RAG가 가능해요** — 글이 많지 않다면 키워드 매칭만으로 충분해요. 인프라를 단순하게 유지하는 게 개인 프로젝트에서는 더 중요해요.

**스트리밍이 항상 정답은 아니에요** — 응답이 빠른 모델이라면 JSON + 애니메이션 조합이 더 깔끔할 수 있어요.

**코드베이스 컨텍스트가 정확도를 높여요** — 게시글 내용만으로는 실제 구현 상태와 차이가 생길 수 있어서, 빌드 타임에 코드베이스 현황을 함께 제공하면 답변이 훨씬 정확해져요.

**애니메이션은 단순한 게 자연스러워요** — 글자 단위, 스트리밍 방식 등 여러 가지를 시도했는데 결국 문단별 fade-in이 가장 자연스러웠어요.

이 블로그 우측 하단의 채팅 버튼을 눌러서 직접 써볼 수 있어요. 블로그 관련 질문을 해보세요!
