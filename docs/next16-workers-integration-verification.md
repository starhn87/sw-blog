...
현재 요청량에는 여유가 있지만, **전체 기능의 Workers Free 안정성은 아직 통과하지 못했다.**
검색·Claude streaming·원격 D1/R2·벡터 쓰기는 동작했다. 다만 챗봇과 D1 읽기/쓰기 혼합
구간에서 CPU 10 ms 초과가 관측됐다. 네이버 지도는 운영에서 정상이고 Preview에서
인증 실패가 표시됐다. 실제 기기 푸시 수신과 전환용 배포/재인덱싱 workflow도 남아 있다.
...
1. 챗봇의 Next.js 경유 비용과 스트리밍/후속 작업을 분리 측정하고 최적화 후보를 검증한다.
   D1도 endpoint별로 재측정해 10 ms 초과가 어느 처리에서 생기는지 구분한다.
2. 네이버 지도 Preview 인증을 확인하고, 테스트 기기에서 실제 푸시 수신을 검증한다.
3. 예약 문자·UTF-8 등 남은 Next fallback의 CPU 위험을 기존 기록과 함께 검토한다.
4. Workers 배포 방식과 배포 성공 commit에 연결된 재인덱싱 workflow를 구현·검증한다.
   현재 `.github/workflows/reindex.yml`은 여전히 Pages production 배포를 기다린다.
5. 위 항목 해결 후 승인받아 운영 전환하고, 최소 7일간 오류·CPU·사용량·SEO를 관찰한다.
   기존 Pages 성공 배포를 rollback 원천으로 보존한다.
